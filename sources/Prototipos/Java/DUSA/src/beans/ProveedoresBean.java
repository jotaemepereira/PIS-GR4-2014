	...
	public void altaProveedor() {
		...
		if (!RUT.equals("")) {
			if (tipoDocumento.equals("C")) { // en caso de ci
				...
				int suma = 0;
				/*
				 * Para el caso en el que la CI tiene menos de 8 digitos calculo
				 * cuantos coeficientes no voy a usar
				 */
				int difCoef = arrCoefs.length - RUT.length();
				/*
				 * recorro cada digito empezando por el de más a la derecha o
				 * sea, el digito verificador, el que tiene indice mayor en el
				 * array
				 */
				for (int i = (RUT.length() - 1); i > -1; i--) {
					// Obtengo el digito correspondiente de la ci recibida
					String dig = RUT.substring(i, i + 1);
					// Lo tenía como caracter, lo transformo a int para poder
					// operar
					int digInt = Integer.parseInt(dig);
					// Obtengo el coeficiente correspondiente al ésta posición
					// del digito
