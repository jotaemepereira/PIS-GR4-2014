		...
		try {
			if (motivo.trim().isEmpty()) {
				...
				if (cambios > 0) {
					...
					contexto.addMessage(null, new FacesMessage(
							FacesMessage.SEVERITY_INFO,
							"Cambio de stock realizado con �xito.", ""));
		...
		try {
			if (nuevoStockSeleccionado >= articuloSeleccionado.getStock()) {
				contexto.addMessage(
						...
						new FacesMessage(
								FacesMessage.SEVERITY_ERROR,
								"El nuevo stock del art�culo origen debe ser menor al actual.",
								""));
				...
				contexto.addMessage(
						...
						new FacesMessage(
								FacesMessage.SEVERITY_ERROR,
								"El nuevo stock del art�culo destino debe ser mayor al actual.",
								""));
			} else {
				IStock is = FabricaLogica.getIStock();
