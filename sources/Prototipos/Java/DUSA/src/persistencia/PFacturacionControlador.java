	...
	public List<Venta> listarVentasPendientes() throws Exception {
		try {
			...
			String sqlQuery = "SELECT * FROM sales s "
							...
							+ "WHERE s.sale_status = '" + Enumerados.EstadoVenta.PENDIENTE + "'";
            ...
            while (rs.next()){
            	if (v == null || rs.getLong("sale_id") != v.getVentaId()) {
            		...
            		v.setMontoNetoGravadoIvaBasico(rs.getBigDecimal("taxed_basic_net_amount"));
            		v.setMontoNetoGravadoIvaMinimo(rs.getBigDecimal("taxed_minimum_net_amount"));
            		v.setMontoNoFacturable(rs.getBigDecimal("not_billable_amount"));
            		v.setMontoNoGravado(rs.getBigDecimal("not_taxed_amount"));
            		v.setMontoRetenidoIRAE(rs.getBigDecimal("irae_withheld_amount"));
            		v.setMontoRetenidoIVA(rs.getBigDecimal("iva_withheld_amount"));
            		v.setMontoTotal(rs.getBigDecimal("total_amount"));
            		v.setMontoTotalAPagar(rs.getBigDecimal("total"));
            		v.setMontoTributoIvaBasico(rs.getBigDecimal("taxed_basic_amount"));
            		v.setMontoTributoIvaMinimo(rs.getBigDecimal("taxed_minimum_amount"));
